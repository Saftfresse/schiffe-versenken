...
namespace WA_Schiffe_Versenken
{
	public partial class Form1 : Form
	{
		...
		List<Point> locationValid(Point p, int length, int dir, bool enemy = true)
		{
			bool ret = true;
				...
				for (int i = 0; i < length; i++)
				{
					switch (dir)
					{
						case 1:
							try
							{
								if (userCells[p.X, p.Y + i] == CellType.Wasser)
								{
									Point pt = new Point(p.X, p.Y + i);
									points.Add(pt);
								}
						...
						case 2:
							try
							{
								if (userCells[p.X - i, p.Y] == CellType.Wasser)
								{
									Point pt = new Point(p.X - i, p.Y);
									points.Add(pt);
								}
		...
		Rectangle getCurrentCell()
		{
			...
			for (int i = 0; i < userCells.GetLength(0); i++)
			{
				...
				{
					Rectangle tmp = new Rectangle(j * length , i * length , length,length);
					if (tmp.Contains(MLoc))
					{
						r = new Rectangle(j,i,1,1);
					}
				}
			}
			return r;
		...
		List<Point> getValidPoints(Rectangle _base, shipType _ship)
		{
			int length = canvas_player.Size.Width / userCells.GetLength(0);
			...
			if (horizontal)
			{
				while (_base.X > userCells.GetLength(1) - _ship.Length)
				{
					_base.X--;
				}
				for (int i = 0; i < _ship.Length; i++)
				{
					ret.Add(new Point(_base.X + i, _base.Y));
				}
			}
			else
			{
				while (_base.Y > userCells.GetLength(0) - _ship.Length)
				{
					_base.Y--;
				}
				for (int i = 0; i < _ship.Length; i++)
				{
					ret.Add(new Point(_base.X, _base.Y + i));
				}
			}
			return ret;
		}

		List<Rectangle> getValidCells(Rectangle _base, shipType _ship)
		{
			int length = canvas_player.Size.Width / userCells.GetLength(0);
			...
			if (horizontal)
			{
				...
				for (int i = 0; i < _ship.Length; i++)
				{
					ret.Add(new Rectangle(_base.X + i, _base.Y, 1, 1));
				}
			}
			else
			{
				...
				for (int i = 0; i < _ship.Length; i++)
				{
					ret.Add(new Rectangle(_base.X, _base.Y + i , 1, 1));
				}
			}
			...
			using (var g = Graphics.FromImage(userSide))
			{
				...
				for (int i = 0; i < userCells.GetLength(0); i++)
				{
					for (int j = 0; j < userCells.GetLength(1); j++)
					{
						...
						if (!getShipAtPoint(new Point(i, j), true).Destroyed)
						{
				...
				if (pickedShip.Name.Length > 0 && canvas_player.Bounds.Contains(PointToClient(MousePosition)))
				{
					Rectangle r = getCurrentCell();
					List<Rectangle> cells = getValidCells(r, pickedShip);
					for (int i = 0; i < cells.Count; i++)
					{
						Rectangle abs = new Rectangle(cells[i].X * length, cells[i].Y * length, length, length);
						g.FillRectangle(new SolidBrush(Color.FromArgb(100, pickedShip.Color)), abs.X , abs.Y , length, length);
					}
				}
			}
		...
		Ship getShipAtPoint(Point p, bool enemy = true)
		{
			Ship s = new Ship();
			if (enemy)
			{
		...
		Point locationToCell(Point mouseLoc, bool enemy = true)
		{
			Point ret = new Point();
			if (enemy)
			{
			...
			{
				int length = canvas_player.Size.Width / userCells.GetLength(1);
				int currX = 0, currY = 0;
				for (int i = 0; i < userCells.GetLength(0); i++)
				{
					for (int j = 0; j < userCells.GetLength(1); j++)
		...
		private void Form1_Load(object sender, EventArgs e)
		{
			...
			shipTypes.Add(new shipType() { Name = "Schlachtschiff", Amount = 1, Length = 5, Id = 1, Color = Color.FromArgb(40, 40, 40) });
			shipTypes.Add(new shipType() { Name = "Kreuzer", Amount = 2, Length = 4, Id = 2, Color = Color.IndianRed });
			shipTypes.Add(new shipType() { Name = "Zerstörer", Amount = 3, Length = 3, Id = 3, Color = Color.PaleGreen });
			shipTypes.Add(new shipType() { Name = "UBoot", Amount = 4, Length = 2, Id = 4, Color = Color.DarkOrange });

			userSide = new Bitmap(canvas_player.Size.Width, canvas_player.Size.Height);
			pickedShip = new shipType() { Name = "", Length = 0, Amount = 0 };
			generateEnemy();
		}
		...
		private void pictureBox2_Paint(object sender, PaintEventArgs e)
		{
			...
			for (int i = 0; i < enemyCells.GetLength(0); i++)
			{
				for (int j = 0; j < enemyCells.GetLength(1); j++)
				{
					...
					if (enemyCells[i, j] == CellType.Schlachtschiff) c = shipTypes.Find(x => x.Name == enemyCells[i, j].ToString()).Color;
		...
		private void canvas_player_MouseClick(object sender, MouseEventArgs e)
		{
			MLoc = e.Location;
			Ship s = new Ship();
			s.Name = pickedShip.Name;
			s.Length = pickedShip.Length;
			s.ShipId = pickedShip.Id;
			Rectangle r = getCurrentCell();
			Console.WriteLine(r);
			List<Point> pp = locationValid(r.Location, pickedShip.Length, horizontal?1:4, false);
			for (int i = 0; i < pp.Count; i++)
			{
				userCells[pp[i].Y, pp[i].X] = (CellType)pickedShip.Id;
				s.Points[getValidPoints(getCurrentCell(), pickedShip).IndexOf(pp[i])] = new Point(pp[i].X, pp[i].Y);
				Console.WriteLine(userCells[pp[i].Y, pp[i].X]);
			}
		}
		...
		{
			pickedShip = shipTypes.Find(x => x.Name == CellType.Schlachtschiff.ToString());
		}

		private void btn_pick_2_Click(object sender, EventArgs e)
		{
			pickedShip = shipTypes.Find(x => x.Name == CellType.Kreuzer.ToString());
		}

		private void btn_pick_3_Click(object sender, EventArgs e)
		{
			pickedShip = shipTypes.Find(x => x.Name == CellType.Zerstörer.ToString());
		}

		private void btn_pick_4_Click(object sender, EventArgs e)
		{
			pickedShip = shipTypes.Find(x => x.Name == CellType.UBoot.ToString());
		}
